...
import { FaUsers, FaDiscord, FaSpotify } from 'react-icons/fa';
...
function MemberCard({ member }: { member: Member }) {
  ...
  // Activities
  const activity = Array.isArray((member as any).activities) && (member as any).activities.length > 0 ? (member as any).activities[0] : null;
  // Spotify
  const spotify = (member as any).spotify;
    ...
    >
      ...
      <div className="flex items-center gap-4 w-full md:w-auto">
        <div className="relative">
          <img
            src={getAvatarUrl(member.discord_user.id, member.discord_user.avatar)}
            alt={member.discord_user.username}
            className="w-20 h-20 rounded-full object-cover border-2 border-white dark:border-gray-800"
          />
          <span className={`absolute bottom-1 right-1 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 ${statusColor}`}></span>
        </div>
        <div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg md:text-xl text-white dark:text-white">
              {member.discord_user.global_name || member.discord_user.display_name}
            </span>
            {member.discord_user.bot && (
              <span className="ml-1 px-2 py-0.5 text-xs rounded-full bg-blue-900 text-blue-200 font-semibold">Bot</span>
            )}
          </div>
          <div className="text-gray-300 text-sm mb-2">@{member.discord_user.username}</div>
          <div className="flex gap-2 mt-1">
            {platforms.map((p) => (
              <span key={p} className="bg-neutral-800 text-white text-xs px-3 py-1 rounded-lg font-semibold border border-neutral-700">
                {p}
              </span>
            ))}
          </div>
      ...
      <div className="flex flex-col items-start md:items-end justify-center w-full md:w-auto">
        ...
        {spotify && (
          <div className="flex items-center mt-2">
            ...
            <div>
              <div className="text-green-400 font-bold text-base leading-tight">{spotify.song}</div>
