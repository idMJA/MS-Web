  ...
  return (
    <main className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white py-10 pt-30">
      ...
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold mb-8 text-center">Top Voice Leaderboard</h1>
        ...
        {loading ? (
          <div className="overflow-x-auto">
            <table className="min-w-full border border-gray-300 dark:border-gray-700 rounded-lg overflow-hidden" style={{ background: 'transparent' }}>
              <thead className="bg-gray-100 dark:bg-gray-800 bg-opacity-60 dark:bg-opacity-60" style={{ background: 'rgba(30,41,59,0.6)' }}>
                <tr>
            ...
            <table className="min-w-full border border-gray-300 dark:border-gray-700 rounded-lg overflow-hidden" style={{ background: 'transparent' }}>
              ...
              <tbody>
                {users.map((user, idx) => {
                  ...
                  let rowClass = "";
                  let style: React.CSSProperties = { background: 'transparent' };
                  if (idx < 5) {
                    rowClass = "backdrop-blur-md border border-yellow-400/40";
                    style = {
                      background: "linear-gradient(90deg, rgba(255,215,0,0.18) 0%, rgba(255,255,255,0.08) 100%)",
                  ...
                  return (
                    <tr key={user.user_id} className={rowClass} style={style}>
                      ...
                      <td className="py-2 px-4 flex items-center gap-2">
                        <Image src={user.avatar ? `https://cdn.discordapp.com/avatars/${user.user_id}/${user.avatar}.png` : "https://cdn.discordapp.com/embed/avatars/0.png"} alt={user.display_name || user.username} width={32} height={32} className="rounded-full" />
                        <span>{user.display_name || user.username}</span>
                      </td>
                      <td className="py-2 px-4 text-center">{user.duration > 0 ? formatDuration(user.duration) : null}</td>
                    </tr>
                  );
                })}
              </tbody>
